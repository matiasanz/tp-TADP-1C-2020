
class RefreshException < StandardError
  def initialize(objeto)
    super("No es posible ejecutar refresh!(), el objeto de la clase " + objeto.class.name + " no posee id")
  end
end

class ForgetException < StandardError
  def initialize(objeto)
    super("No es posible ejecutar forget!(), el objeto de la clase " + objeto.class.name + " no posee id")
  end
end

class TipoDeDatoException < StandardError
  def initialize(clase, atributo, tipo)
    super("El atributo \"" + atributo.to_s + "\" de la clase \"" + clase.name +
              "\" no corresponde con el tipo declarado. Debe ingresar :" + tipo.name)
  end
end

class NoBlankException < StandardError
  def initialize(clase, atributo)
    super("El atributo \"" + atributo.to_s + "\" de la clase \"" + clase.name + "\" no esta inicializado.")
  end
end

class FromException < StandardError
  def initialize(clase, atributo, from)
    super("El atributo \"" + atributo.to_s + "\" de la clase \"" + clase.name + "\" debe ser mayor que " + from.to_s)
  end
end

class ToException < StandardError
  def initialize(clase, atributo, to)
    super("El atributo \"" + atributo.to_s + "\" de la clase \"" + clase.name + "\" debe ser menor que " + to.to_s)
  end
end

class BlockValidateException < StandardError
  def initialize(clase, atributo, proc)
    super("El atributo \"" + atributo.to_s + "\" de la clase \"" + clase.name + "\" no cumple con: #{proc}")
  end
end