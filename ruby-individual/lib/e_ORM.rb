    ...
    module ModuloPersistible
        ...
        def method_missing(method, *args)
            if trying_to_find?(method)
                ...
                value = args.first
                condicion = value.is_a?(Proc)? value : proc{|v| v==value}

                #TODO Esto era en primer lugar, que si el atributo ya estuviera en la tabla,
                # que no hubiera que instanciar todos los elementos. Especificamente tenia sentido en
                # el caso particular de que una clase estuviera compuesta por un atributo de si misma, lo cual al ir a buscar por id
                # genera una recursividad infinita. De ahi el test que falla. Despreciando ese caso, la nueva implementacion funciona
                # Pero descomentando esta linea fallan los mixines, no pude determinar por que.
                # Si, ya se que no tenia que contemplar ese caso, pero fui cabeza dura :P
                #if entrada_de_tabla?(property)
                #    tabla.find_by(property, condicion) + @submodulos_inmediatos.flat_map{|m|m.send(method, args)}
                #else
                    all_instances.select{|i| i.send(property).instance_eval(&condicion)}
                #end
            else
                super
            end
