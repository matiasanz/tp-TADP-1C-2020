...
case class CaraCruz(valorElegido: ValorCaraCruz, monto: Int) {
  def apply(valorObtenido: ValorCaraCruz): Int = if (valorElegido == valorObtenido) monto * 2 else 0
}
...
case class Ruleta(monto: Int, seJugoA: JugadaRuleta) extends Jugada {
  // TODO, deberia limitar el numero de 0 a 36? como se tira excepcion en constructor?
  override def apply(numeroObtenido: Int): Int = seJugoA match {
    ...
    case ADocena(docenaElegida) => if (esDocena(docenaElegida, numeroObtenido)) monto * 3 else 0
  }

  private def esDocena(docenaElegida: Int, numeroObtenido: Int): Boolean = {
